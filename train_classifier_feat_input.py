...
sys.path.append(os.path.dirname(__file__) + "/../..")
from utils import save_to_json, init_logging, ensure_dir
sys.path.remove(os.path.dirname(__file__) + "/../..")
import logging
...
class Model(nn.Module):
    def __init__(self, args, emb_layer, nclasses=2, feat_input=False, feat_dim=100):
        # if feat_input == True, we take in feature instead of token ids.
        # in this case feat_dim specifies the dimensionality of the representation.
        ...
        if args.cnn:
            ...
            d_out = args.d * 2 # bi directional lstm
        elif args.la:
            if self.feat_input:
                d_out = feat_dim
            else:
    ...
    def forward(self, input):
        if self.feat_input:
            # if feat_input we assume the input shape would be 
            # (sequence length x batch size x feat dim)
            if self.args.cnn:
                raise Exception("feat input mode does not support cnn model yet!")
        else:
            if self.args.cnn:
                input = input.t()

        # get emb
        if not self.feat_input:
        ...
        if self.args.cnn:
            ...
            output = output.sum(dim=0) / float(emb.size()[0])
        else:
            output = emb.sum(dim=0) / float(emb.size()[0])
        if not self.args.la:
...
def eval_model(model, valid_x, valid_y, pred_file=None, prob_file=None):
    ...
    model.train()
    if torch.__version__ >= '0.4':
        return 1.0-float(correct)/float(cnt), np.array(preds)
    else:
        return 1.0-correct/cnt, np.array(preds)
...
def train_model(epoch, model, optimizer,
    ...
    valid_err, _ = eval_model(model, valid_x, valid_y)

    if torch.__version__ >= "0.4":
        # for debugging purpose
        # print("double check ", epoch, niter,
    ...
    if valid_err < best_valid:
        ...
        test_err, test_pred = eval_model(model, test_x, test_y, pred_file=pred_file, prob_file=prob_file)
        # Save model
        if save_mdl is not None:
            torch.save(model, save_mdl)
        # if out folder is not None, we save the best prediction so far
        np.save(out_folder + "/best_test_pred.npy", test_pred)
    else:
        test_pred = None
    return best_valid, test_err, test_pred

def cyclic_lr(initial_lr, iteration, epoch_per_cycle):
    return initial_lr * (math.cos(math.pi * iteration / epoch_per_cycle) + 1) / 2

def save_final_results(args, best_valid, test_err, test_pred):
    results = args.__dict__
    results["results"] = {"best_valid_err": best_valid, "test_err": test_err, "test_pred": test_pred.tolist()}
    save_to_json(results, args.out + "/final_results.json")

def main(args):
    if args.feat_input:
        train_x, train_y, valid_x, valid_y, test_x, test_y = dataloader.read_npy_dataset(args.feat_input_folder, args.dataset)
    else:
        train_x, train_y, valid_x, valid_y, test_x, test_y = dataloader.read_split_dataset(args.path, args.dataset)
    data = train_x + valid_x + test_x

    # the embedding constructed in the following list is only valid for the token input mode
    if not args.feat_input:
        if args.embedding:
    ...
    if not args.feat_input:
        ...
        train_x, train_y, train_len = dataloader.create_batches_feat_input(
            train_x, train_y,
            args.batch_size,
            sort = 'sst' in args.dataset
        )
        valid_x, valid_y, valid_len = dataloader.create_batches_feat_input(
            valid_x, valid_y,
            args.batch_size,
            sort = 'sst' in args.dataset
        )
        test_x, test_y, test_len = dataloader.create_batches_feat_input(
            test_x, test_y,
            args.batch_size,
            sort = 'sst' in args.dataset
        )
    ...
    if args.load_mdl is None:
        model = Model(args, emb_layer, nclasses, feat_input=args.feat_input, feat_dim=args.feat_dim).cuda()
    else:
    ...
    if not args.snapshot:
        for epoch in range(args.max_epoch):
            best_valid, test_err, test_pred_tmp = train_model(epoch, model, optimizer,
                train_x, train_y,
                valid_x, valid_y,
                test_x, test_y,
                best_valid, test_err,
            ...
            if test_pred_tmp is not None:
                # test pred tmp is only None when it is currently the best ckpt
                test_pred = test_pred_tmp
            save_final_results(args, best_valid, test_err, test_pred)
    # Snapshot ensembling
    else:
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--path", type=str, help="path to corpus directory")
    ...
    argparser.add_argument("--feat_input", action="store_true", help="Use feature instead of token as input")
    argparser.add_argument("--feat_input_folder", type=str, help="The npy file folder to load feature and label from.")
    argparser.add_argument("--feat_dim", type=int, help="the dimensioality of input")
    args = argparser.parse_args()

    # save output log to the out folder
    ensure_dir(args.out)
    init_logging(args.out)

    # Dump git hash
    h = check_output(['git', 'rev-parse', '--short', 'HEAD']).strip()
    logging.info("Git hash: " + str(h))
